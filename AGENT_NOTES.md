...
## Production hotfix — Codex handoff (2026-08-29)

- **Handoff reconciliation.** The older note that Phase 4.5 was uncommitted is stale: the repository was clean at `db68cc3` (`fix: add Vercel API transport`) and Vercel had deployed that commit. Codex took over the subsequent runtime hotfix.
- **Confirmed production failure.** Before the fix, `GET /api/metrics` and `POST /api/guide` at `https://rti-one.vercel.app` both returned HTTP 500 `FUNCTION_INVOCATION_FAILED`; `/dashboard` hard refresh returned 404. The Vercel runtime evidence identified Node ESM resolving `/var/task/server/apiHandlers` without an extension from `api/guide.js`.
- **Confirmed root cause.** The thin serverless wrappers and their complete server-reachable module graph used extensionless local specifiers. Vite tolerated them, but the deployed Node ESM runtime required the emitted `.js` specifiers, causing initialization to fail before handlers ran.
- **Exact fix.** Added explicit `.js` local specifiers throughout the serverless runtime graph (`api/`, `server/`, and their imported intelligence/knowledge modules); changed the Vite config's server import to `.js`; and added `vercel.json` with a SPA rewrite from `/((?!api(?:/|$)).*)` to `/index.html`, deliberately excluding every `/api` path.
- **Local verification.** `npm run test` passed: 64 tests / 12 files, offline. `npm run build` passed. Vite transport smoke: `GET /api/metrics` 200; short `POST /api/interpret` 200 with the safe `mode:"fallback", reason:"too-short"` response. `git diff --check` passed. Client bundle/source scan found no `GROQ_API_KEY`, key prefix, Groq endpoint, or authorization token; `.env` is not tracked. Non-test server import audit found all local runtime specifiers ending in `.js`.
- **Vercel deployment and verification.** Deployed as production `dpl_6hWB8DVnBGwWhWBXRZ7bQ976W13v` (`https://rti-4q84we5zr-rohitisawesome5-9210s-projects.vercel.app`), then aliased to `rti-one.vercel.app`. `GET /api/metrics` 200; short `POST /api/interpret` 200 valid fallback JSON; one bounded `POST /api/guide` 200 with `mode:"ai"` and route `central-railways`; one bounded `POST /api/draft` 200 with `mode:"ai"`; no `FUNCTION_INVOCATION_FAILED`. Vercel runtime-error clusters reported none after deployment.
- **SPA result.** Direct requests to `/dashboard`, `/rti/new`, `/cases`, `/help`, and `/applications/RTI/ONE/2026/00001` all returned 200 `text/html`; `/api/*` remains excluded from the SPA rewrite.
- **Remaining risks.** Existing in-memory request-manager counters remain per warm serverless instance, not global. Browser network behavior was audited from client source and bundle rather than a production browser session in this handoff; the client continues to use same-origin `/api/*` and contains no Groq endpoint.
- **Git status.** The verified hotfix is intentionally uncommitted. Do not commit unless an explicit Git checkpoint instruction is given.
